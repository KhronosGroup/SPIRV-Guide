...
For `OpVariable`, there is an optional `Data Type` in the `OpUntypedVariableKHR` to check. If you are using `Shader` (Vulkan) then this is [required the be there](https://docs.vulkan.org/spec/latest/appendices/spirvenv.html#VUID-StandaloneSpirv-OpUntypedVariableKHR-11167). Vulkan also requires the untyped variable storages classes to be explicitly laid out (`StorageBuffer`, `PhysicalStorageBuffer`, `Uniform`, `PushConstant`, `Workgroup`, etc).
